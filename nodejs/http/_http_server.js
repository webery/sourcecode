...
const HTTPParser = process.binding('http_parser').HTTPParser;//解析http请求
...
util.inherits(ServerResponse, OutgoingMessage);//继承OutgoingMessage
...
exports.ServerResponse = ServerResponse;//暴露Response对象
...
function Server(requestListener) {
  ...
  if (requestListener) {
    this.addListener('request', requestListener);//注册'request'事件
  }
  ...
  this.addListener('connection', connectionListener);//注册net.Server连接事件
...
exports.Server = Server;//暴露Server对象
...
function connectionListener(socket) {
  ...
  parser.onIncoming = parserOnIncoming;//注册parserOnIncoming到parser中,当parser完成解析,调用parserOnIncoming
  ...
  //解析http请求,生成request和Response对象,并触发在服务器listener注册的监听器
  function parserOnIncoming(req, shouldKeepAlive) {
    ...
    var res = new ServerResponse(req);//生成Response对象
    ...
    if (req.headers.expect !== undefined &&
      ...
      if (EventEmitter.listenerCount(self, 'checkContinue') > 0) {
        ...
        res.writeContinue();
        self.emit('request', req, res);//触发在server注册的listener事件
      }
    } else {
