...
public class RequestMappingHandlerAdapter extends AbstractHandlerMethodAdapter
	...
	private HandlerMethodArgumentResolverComposite argumentResolvers;//自带参数解析器

	private HandlerMethodArgumentResolverComposite initBinderArgumentResolvers;

	private List<HandlerMethodReturnValueHandler> customReturnValueHandlers;//自定义返回值解析器

	private HandlerMethodReturnValueHandlerComposite returnValueHandlers;//自带参数解析器

	private List<ModelAndViewResolver> modelAndViewResolvers;//模型视图解析器

	private ContentNegotiationManager contentNegotiationManager = new ContentNegotiationManager();

	private List<HttpMessageConverter<?>> messageConverters;//报文转换器
	...
	private ModelAndView invokeHandleMethod(HttpServletRequest request,
		...
		WebDataBinderFactory binderFactory = getDataBinderFactory(handlerMethod);//取得databinder工厂
		ModelFactory modelFactory = getModelFactory(handlerMethod, binderFactory);//取得Model工厂
		//把处理器封装成ServletInvocableHandlerMethod
		ServletInvocableHandlerMethod requestMappingMethod = createRequestMappingMethod(handlerMethod, binderFactory);
		//生成模型视图容器,保存一些模型和视图相关的东西
		...
		//准备工作完成,开始调用ServletInvocableHandlerMethod进入主题
		...
		return getModelAndView(mavContainer, modelFactory, webRequest);//根据处理过程收集到的信息,生成模型视图
	}
	...
	private ServletInvocableHandlerMethod createRequestMappingMethod(
		...
		//把处理器封装成ServletInvocableHandlerMethod,这样就可以实现处理器的可以调用,
		//并且可以构造参数,处理返回值等
