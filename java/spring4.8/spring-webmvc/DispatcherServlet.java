...
public class DispatcherServlet extends FrameworkServlet {
	...
	//DispacherServlet使用了下面这些常量作为组件在IOC的固定名字,也就是ID
	...
	private LocaleResolver localeResolver;//本地化解析器

	/** ThemeResolver used by this servlet */
	private ThemeResolver themeResolver;//主题解析器

	/** List of HandlerMappings used by this servlet */
	private List<HandlerMapping> handlerMappings;//hm

	/** List of HandlerAdapters used by this servlet */
	private List<HandlerAdapter> handlerAdapters;//ha
	...
	protected void doDispatch(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		HandlerExecutionChain mappedHandler = null;//保存处理链(包括拦截器和处理器)
		boolean multipartRequestParsed = false;//标记当前请求是否是文件上传
		...
		try {
			...
			try {
				//如果是文件上传请求,那么将会生成一个不一样的request.实际上使用的是common upload这个第三方jar
				processedRequest = checkMultipart(request);
				multipartRequestParsed = (processedRequest != request);

				// Determine handler for the current request.
				//遍历hm数组, 找到可以url对应的处理器,然后注入拦截器,然后把它俩封装返回
				...
				//遍历ha数组获取合适的ha
				HandlerAdapter ha = getHandlerAdapter(mappedHandler.getHandler());//获取ha
				...
				try {
					...
					//ha调用处理器,1:给处理器生成参数,这个过程包括数据类型转换格式化,数据绑定和数据校验等
				    //2:使用1中的参数通过反射调用处理器;
				    //3:处理处理器返回的结果,如果是使用了@ResponseBody会把数据刷入响应流中
					//4:返回mv, 如果返回的不是视图, 那么mv == null
			...
			}//处理返回的视图,视图渲染在这里发生 当时候@ResponseBody这样的注解的时候,是不返回视图的
			...
			if (multipartRequestParsed) {
				//知道我们为什么前面需要使用一个变量来标记当前请求是上传文件了,因为文件上传可能占用不少的内存,需要释放一下
	...
	protected HandlerExecutionChain getHandler(HttpServletRequest request) throws Exception {
		//遍历hm数组,直到第一个hm可以根据当前request(url, method, header等,
		//也就是我们在控制器方法使用的@RequestMapping使用的参数)可以解析HandlerExecutionChain出来,包括了拦截器和处理器
	...
	protected HandlerAdapter getHandlerAdapter(Object handler) throws ServletException {
		//遍历ha, 直到找到第一个可以处理处理链的ha. 和找hm类似, 实际上, 在SpringMVC中很多地方都重要了这样的方法,后面会看到
